...
userdata <- read.csv("C:/Users/lawrenceb/Kent Wildlife Trust/EVD - Bugs Matter - General/10. Reporting/1. Analysis/Analysis 2025/bugsmatter-users-6c5029fa-9fc0-4ed8-a7a9-86fa814ea16d.csv")
...
userdata_w_postcodes1 <- userdata_w_postcodes %>%
  ...
  dplyr::select(User.ID, w_postcode) %>%
...
users_manual_postcodes <- read.csv("C:/Users/lawrenceb/Kent Wildlife Trust/EVD - Bugs Matter - General/10. Reporting/1. Analysis/Analysis 2025/users_manual_postcodes.csv")
users_manual_postcodes1 <- users_manual_postcodes %>% dplyr::select(User.ID, postcode)
...
plot(userdata5[7])
st_crs(userdata5)
summary(is.na(userdata5$postcode))
...
summary(as.factor(userdata6$region))
summary(as.factor(userdata6$country))
...
st_write(obj = userdata8, dsn = pool, Id(schema="bugs_matter", table = "user_data3"), append=FALSE)
